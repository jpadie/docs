...
ZbName<a class="cmnd" id="zbname"></a>|Sets or reads the Zigbee device friendly name (up to 32 characters).<BR>`<device>,<name>` sets the new friendly name<BR>`<device>,` (empty name) clears the friendly name<BR>`<device>,<name>,<endpoint name>,<ep>` sets a friendly name for a specified endpoint.  e.g. a switch with 2 gangs having a friendly name of "switch" could have its gangs named `ZbName switch,Gang 1,1` and `ZbName switch,Gang 2,2`<BR>`<device>` displays the current friendly name<BR>Also see [`SetOption83 1`](#setoption83) to enable friendly names as JSON keys instead of ShortAddr.
